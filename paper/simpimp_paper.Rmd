...
alpha.tab <- as.data.frame(d_alpha_all_summary) %>%
  ...
  rename("Age bin" = age_group,
         "M trials" = n,
         "alpha acc" = acc_raw_alpha,
         "alpha rt" = rt_raw_alpha
         )

apa_table(alpha.tab, caption = "Standardized reliability coefficients (Cronbach\'s alpha) for accuracy (Acc) and reaction time (RT) by age group, along with the mean number of trials for each")
```
